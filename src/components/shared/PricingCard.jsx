    ...
    return (
        <div className="relative rounded-3xl p-8 border border-amber-500/30 bg-gradient-to-b from-slate-900/80 via-slate-900/40 to-slate-950 flex flex-col justify-between shadow-[0_0_40px_rgba(245,158,11,0.08)] group overflow-hidden">
            ...
            <form
                ...
                method="POST"
                onSubmit={() => setLoading(true)} // Keeps the loading spinner active during form submission redirect
                className="w-full mt-8"
            >
                {/* 💡 Crucial: Hidden input fields to send data safely to your Express backend */}
                <input type="hidden" name="userId" value={userId} />
                <input type="hidden" name="priceAmount" value="1500" />
                ...
                <button
                    ...
                    role="link"
                    disabled={loading}
                    className="w-full group/btn relative flex items-center justify-center gap-2 px-5 py-3.5 text-sm font-bold text-slate-950 bg-gradient-to-r from-cyan-400 via-amber-400 to-orange-500 rounded-xl shadow-[0_0_20px_rgba(245,158,11,0.1)] hover:shadow-[0_0_30px_rgba(249,115,22,0.3)] hover:scale-[1.01] transition-all duration-300 active:scale-[0.99] font-black uppercase tracking-wider cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed"
                >
                    {loading ? (
