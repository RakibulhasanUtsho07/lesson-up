...
import { Button, Card, Avatar, Input } from "@heroui/react";
import { 
  FiMail, 
  FiBookOpen, 
  FiBookmark, 
        ...
        <div className="flex items-center justify-between border-t border-slate-900/80 pt-4">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1 text-xs font-bold text-slate-400 group-hover:text-rose-400 transition-colors">
              <FiHeart className="group-hover:fill-rose-500/10" /> {lesson.reactions || 0}
            </span>
            <span className="flex items-center gap-1 text-xs font-bold text-slate-400 group-hover:text-amber-400 transition-colors">
              <FiBookmark className="group-hover:fill-amber-500/10" /> {lesson.saves || 0}
            </span>
          </div>
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 p-6 flex items-center justify-center">
        <div className="animate-pulse flex flex-col items-center gap-2">
          <div className="size-8 rounded-full border-2 border-t-cyan-500 animate-spin border-slate-800" />
          <p className="text-xs font-bold tracking-widest text-slate-500 uppercase">Synchronizing...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="space-y-4 flex-1 w-full">
              <div>
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 mb-1">
                  ...
                  <span className="flex items-center gap-1.5 text-[10px] font-black tracking-widest uppercase bg-gradient-to-r from-amber-500/10 to-orange-500/10 text-orange-400 border border-orange-500/20 px-3 py-1 rounded-full shadow-inner animate-pulse">
                    Premium ⭐
                  </span>
                </div>

                <p className="text-xs text-slate-500 flex items-center justify-center sm:justify-start gap-1.5 font-medium">
                  <FiMail className="text-slate-600 text-sm" /> {user?.email || "utsho@utsho.com"}
                </p>
              </div>
          ...
          <div className="shrink-0">
            {isEditing ? (
              ...
              <Button 
                ...
                startContent={<FiSettings className="animate-spin-slow text-slate-500" />}
              >
                Configure
              </Button>
            )}
